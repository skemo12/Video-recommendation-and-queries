...
/**
 * Class for favorite video query, used to get favorite video query
 */
public final class FavoriteQuery implements QueryInterface {
     ...
     * according to type
     */
    private List<Show> sortShowsByFavorite(final List<Show> videos,
                                           final String type) {
    ...
    public List<String> createOutputList(final ActionInputData command,
        ...
        List<Show> mostFavoriteVideos = new ArrayList<>();
        ...
        if (command.getObjectType().equalsIgnoreCase("movies")) {
            ...
            for (Movie movie : movies) {
                if (filters.checkShowFilters(movie)
                        && movie.getFavoriteCount() > 0) {
                    mostFavoriteVideos.add(movie);
                }
            }
        } else {
            ...
            for (Serial serial : serials) {
                if (filters.checkShowFilters(serial)
                        && serial.getFavoriteCount() > 0) {
                    mostFavoriteVideos.add(serial);
                }
            }
        }

        mostFavoriteVideos = sortShowsByFavorite(mostFavoriteVideos,
                command.getSortType());
        List<String> outputTitles = new ArrayList<>();
        for (Show video : mostFavoriteVideos) {
